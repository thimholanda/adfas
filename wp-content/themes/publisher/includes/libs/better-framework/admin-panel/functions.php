...
if ( ! function_exists( 'bf_get_option' ) ) {
	...
	function bf_get_option( $key, $panel_key = '', $lang = NULL ) {
		...
		if ( isset( Better_Framework::options()->options[ $panel_key ]['fields'][ $key ] ) ) {

			if ( isset( Better_Framework::options()->options[ $panel_key ]['fields'][ $key ][ $std_id ] ) ) {
				return Better_Framework::options()->options[ $panel_key ]['fields'][ $key ][ $std_id ];
			} elseif ( isset( Better_Framework::options()->options[ $panel_key ]['fields'][ $key ]['std'] ) ) {
				return Better_Framework::options()->options[ $panel_key ]['fields'][ $key ]['std'];
			} else {
				return NULL;
			}
