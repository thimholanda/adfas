...
<article <?php publisher_attr( 'post', publisher_get_prop_class() . ' clearfix listing-item listing-item-blog  listing-item-blog-3 ' . $main_term_class ); ?>>
	...
	<h2 class="title">
		<a <?php publisher_attr( 'post-url' ); ?>><span <?php publisher_attr( 'post-title' ); ?>><?php publisher_echo_html_limit_words( get_the_title(), 140 ); ?></span></a>
	</h2>
	<?php
	...
	if ( publisher_get_prop( 'show-excerpt', TRUE ) ) { ?>
		<div <?php publisher_attr( 'post-lead' ); ?>>
			<?php publisher_the_excerpt( 530, NULL, TRUE, FALSE ); ?>
		</div>
		<?php
	}
