...
class BF_Breadcrumb {
	...
	public function generate( $echo = TRUE ) {
		...
		if ( ! empty( $this->items ) && is_array( $this->items ) ) {
			...
			$breadcrumb .= ( ! empty( $this->args['before'] ) ? "\n\t\t\t" . '<span class="trail-before">' . $this->args['before'] /* escaped before */ . '</span> ' . "\n\t\t\t" : '' );

			/* Add 'browse' label if it should be shown. */
			if ( TRUE === $this->args['show_browse'] ) {
				$breadcrumb .= "\n\t\t\t" . '<span class="trail-browse">' . $this->args['labels']['browse'] /* escaped before */ . '</span> ';
			}
			...
			$separator = ( ! empty( $this->args['separator'] ) ? '<span class="sep">' . $this->args['separator']  /* escaped before */ . '</span>' : '<span class="sep">/</span>' );

			/* Join the individual trail items into a single string. */
			$breadcrumb .= join( "\n\t\t\t {$separator} ", $this->items );

			/* If $after was set, wrap it in a container. */
			$breadcrumb .= ( ! empty( $this->args['after'] ) ? "\n\t\t\t" . ' <span class="trail-after">' . $this->args['after'] /* escaped before */ . '</span>' : '' );
		...
		if ( $echo ) {
			echo $breadcrumb; // escaped before
		} else {
			return $breadcrumb;
		}
	}
	...
	public function default_labels() {
		...
		$labels = array(
			'browse'              => __( 'Browse:', 'publisher' ),
			'home'                => __( 'Home', 'publisher' ),
			'error_404'           => __( '404 Not Found', 'publisher' ),
			'archives'            => __( 'Archives', 'publisher' ),
			/* Translators: %s is the search query. The HTML entities are opening and closing curly quotes. */
			'search'              => __( 'Search results for &#8220;%s&#8221;', 'publisher' ),
			/* Translators: %s is the page number. */
			'paged'               => __( 'Page %s', 'publisher' ),
			/* Translators: Minute archive title. %s is the minute time format. */
			'archive_minute'      => __( 'Minute %s', 'publisher' ),
			/* Translators: Weekly archive title. %s is the week date format. */
			'archive_week'        => __( 'Week %s', 'publisher' ),
	...
	public function do_network_home_link() {
		...
		if ( is_multisite() && ! is_main_site() && TRUE === $this->args['network'] ) {
			$this->items[] = '<a href="' . esc_url( network_home_url() ) . '" title="' . esc_attr( $this->args['labels']['home'] ) . '" rel="home"><span>' . $this->args['labels']['home'] /* escaped before */ . '</span></a>';
		}
	...
	public function do_site_home_link() {
		...
		$this->items[] = '<a href="' . esc_url( home_url( '/' ) ) . '" title="' . esc_attr( get_bloginfo( 'name' ) ) . '"' . $rel . '>' . $label /* escaped before */ . '</a>';
	}
	...
	public function do_posts_page_items() {
		...
		if ( is_paged() ) {
			$this->items[] = '<a href="' . get_permalink( $post_id ) . '" title="' . esc_attr( $title ) . '" >' . $title /* escaped before */ . '</a>';
	...
	public function do_singular_items() {
		...
		if ( $post_title = single_post_title( '', FALSE ) ) {
			...
			if ( 1 < get_query_var( 'page' ) ) {
				$this->items[] = '<a href="' . get_permalink( $post_id ) . '" title="' . esc_attr( $post_title ) . '">' . $post_title /* escaped before */ . '</a>';
	...
	public function do_post_hierarchy( $post_id ) {
		...
		if ( ! empty( $post_type_object->has_archive ) ) {
			...
			$this->items[] = '<a href="' . get_post_type_archive_link( $post_type ) . '">' . wp_kses( $label, bf_trans_allowed_html() ) . '</a>';
		}
	}
				...
				if ( isset( $matches ) ) {
					...
					foreach ( $matches as $match ) {
						...
						if ( ! empty( $post_types ) ) {
							...
							$this->items[] = '<a href="' . get_post_type_archive_link( $post_type_object->name ) . '" title="' . esc_attr( $label ) . '">' . wp_kses( $label, bf_trans_allowed_html() ) . '</a>';
	...
	public function do_minute_hour_archive_items() {
		...
		if ( TRUE === $this->args['show_title'] ) {
			$this->items[] = sprintf( $this->args['labels']['archive_minute_hour'], get_the_time( _x( 'g:i a', 'minute and hour archives time format', 'publisher' ) ) );
		}
	...
	public function do_minute_archive_items() {
		...
		/* Add the minute item. */
		if ( TRUE === $this->args['show_title'] ) {
			$this->items[] = sprintf( $this->args['labels']['archive_minute'], get_the_time( _x( 'i', 'minute archives time format', 'publisher' ) ) );
		}
	}
	...
	public function do_hour_archive_items() {
		...
		/* Add the hour item. */
		if ( TRUE === $this->args['show_title'] ) {
			$this->items[] = sprintf( $this->args['labels']['archive_hour'], get_the_time( _x( 'g a', 'hour archives time format', 'publisher' ) ) );
		}
	}
	...
	public function do_day_archive_items() {
		...
		$year  = sprintf( $this->args['labels']['archive_year'], get_the_time( _x( 'Y', 'yearly archives date format', 'publisher' ) ) );
		$month = sprintf( $this->args['labels']['archive_month'], get_the_time( _x( 'F', 'monthly archives date format', 'publisher' ) ) );
		$day   = sprintf( $this->args['labels']['archive_day'], get_the_time( _x( 'j', 'daily archives date format', 'publisher' ) ) );

		/* Add the year and month items. */
		$this->items[] = '<a href="' . get_year_link( get_the_time( 'Y' ) ) . '" title="' . esc_attr( $year ) . '">' . $year /* escaped before */ . '</a>';
		$this->items[] = '<a href="' . get_month_link( get_the_time( 'Y' ), get_the_time( 'm' ) ) . '" title="' . esc_attr( $month ) . '">' . $month /* escaped before */ . '</a>';

		/* Add the day item. */
		if ( is_paged() ) {
			$this->items[] = '<a href="' . get_day_link( get_the_time( 'Y' ), get_the_time( 'm' ), get_the_time( 'd' ) ) . '" title="' . esc_attr( $day ) . '">' . $day /* escaped before */ . '</a>';
		} elseif ( TRUE === $this->args['show_title'] ) {
			$this->items[] = $day;
		}
	}
	...
	public function do_week_archive_items() {
		...
		$year = sprintf( $this->args['labels']['archive_year'], get_the_time( _x( 'Y', 'yearly archives date format', 'publisher' ) ) );
		$week = sprintf( $this->args['labels']['archive_week'], get_the_time( _x( 'W', 'weekly archives date format', 'publisher' ) ) );
	...
	public function map_rewrite_tags( $post_id, $path ) {
		...
		if ( is_array( $matches ) ) {
			...
			foreach ( $matches as $match ) {
				...
				if ( '%year%' == $tag ) {
					$this->items[] = '<a href="' . get_year_link( get_the_time( 'Y', $post_id ) ) . '">' . sprintf( $this->args['labels']['archive_year'], get_the_time( _x( 'Y', 'yearly archives date format', 'publisher' ) ) ) . '</a>';
				} /* If using the %monthnum% tag, add a link to the monthly archive. */
				elseif ( '%monthnum%' == $tag ) {
					$this->items[] = '<a href="' . get_month_link( get_the_time( 'Y', $post_id ), get_the_time( 'm', $post_id ) ) . '">' . sprintf( $this->args['labels']['archive_month'], get_the_time( _x( 'F', 'monthly archives date format', 'publisher' ) ) ) . '</a>';
				} /* If using the %day% tag, add a link to the daily archive. */
				elseif ( '%day%' == $tag ) {
					$this->items[] = '<a href="' . get_day_link( get_the_time( 'Y', $post_id ), get_the_time( 'm', $post_id ), get_the_time( 'd', $post_id ) ) . '">' . sprintf( $this->args['labels']['archive_day'], get_the_time( _x( 'j', 'daily archives date format', 'publisher' ) ) ) . '</a>';
				...
				elseif ( '%category%' == $tag ) {
					...
					if ( $terms ) {
						...
						$this->items[] = '<a href="' . get_term_link( $term, 'category' ) . '" title="' . esc_attr( $term->name ) . '">' . wp_kses( $term->name, bf_trans_allowed_html() ) . '</a>';
					}
				}
			}
		}
