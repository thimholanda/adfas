...
abstract class Abstract_Renderer {
  ...
  protected function _background_image($url, $x, $y, $width, $height, $style) {
    ...
    $file_dummy = strtr($file_dummy,"\\:","//");
    $p = strtr($_SERVER["DOCUMENT_ROOT"],"\\:","//");
    $l = strlen($p);
    if ( substr($file_dummy,0,$l) == $p) {
      $file_dummy = substr($file_dummy,$l);
    } else {
    ...
    */
    ...
    $is_png = false;
    $file_dummy .= '_'.$bg_width.'_'.$bg_height.'_'.$bg_x.'_'.$bg_y.'_'.$repeat;
    //debugpng
    //if (DEBUGPNG) print '<pre>[_background_image name '.$file_dummy.']</pre>';
    ...
    if ( method_exists( $this->_canvas, "get_cpdf" ) &&
         ...
         $this->_canvas->get_cpdf()->image_iscached($file_dummy) ) {
       $bg = null;
    ...
    if ( !$is_png && method_exists( $this->_canvas, "get_cpdf" ) && 
      ...
      $this->_canvas->get_cpdf()->addImagePng($file_dummy, $x, $this->_canvas->get_height() - $y - $height, $width, $height, $bg);
    } 
